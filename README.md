...
- `/allow <user_id> [<user_id> ...]` — добавить пользователей в allowlist
  в рантайме. Изменения только в памяти, при рестарте подхватывается
  `ALLOWED_USER_IDS` из окружения. Если `ALLOWED_USER_IDS` не задан и
  бот открыт всем, команда ничего не меняет (и сообщает об этом).
